...
class INotification(object):
    ...
    not have an inherent event models.
...
class IProxy(INotifier):
    ...
    Maintain references to one or more pieces of models data.
    Provide methods for manipulating that data.
    Generate C{INotifications} when their models data changes.
    Expose their name as a C{public static const} called C{NAME}, if they are not instantiated multiple times.
    Encapsulate interaction with local or remote services used to fetch and persist models data.
    """
